  ...
  <div :class="['relative rounded-xl border p-3 transition-all', cardClasses]" @click="handleClick">
    ...
    <div class="flex items-start gap-3">
      ...
      <div :class="['min-w-0 flex-1', isAllComplete ? 'pr-6' : '']">
        ...
        <p :class="['mt-1 min-h-8 line-clamp-2 text-xs leading-4', isLocked ? 'text-current' : 'text-muted-foreground']">{{ group.displayDescription }}</p>
      </div>
    </div>

    <div v-if="isAllComplete" class="mt-2 text-xs font-medium">All tiers complete! 🎉</div>
