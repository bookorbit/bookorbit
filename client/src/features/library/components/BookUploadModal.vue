...
import { CheckCircle2, FileUp, Loader2, Plus, RotateCcw, Upload, X, XCircle, BookOpen } from '@lucide/vue'
...
// Controle de Livro Físico / Manual
const isPhysicalBook = ref(false)
const manualTitle = ref('')
const manualAuthor = ref('')
const manualIsbn = ref('')
const isSubmittingManual = ref(false)
const manualSuccessCount = ref(0)
const manualUploadedIds = ref<number[]>([])
...
const canUpload = computed(() => {
  if (isPhysicalBook.value) {
    return selectedLibraryId.value !== undefined && manualTitle.value.trim().length > 0
  }
  return pendingCount.value > 0 && !isUploading.value && selectedLibraryId.value !== undefined
})

const hasFiles = computed(() => files.value.length > 0)
const allDone = computed(() => {
  if (isPhysicalBook.value) return manualSuccessCount.value > 0
  return hasFiles.value && files.value.every((f) => f.status === 'done' || f.status === 'error')
})
const allSuccess = computed(() => {
  if (isPhysicalBook.value) return manualSuccessCount.value > 0
  return allDone.value && errorCount.value === 0
})
...
function handleClose() {
  const allIds = [...uploadedBookIds.value, ...manualUploadedIds.value]
  if (allIds.length > 0) emit('uploaded', allIds)
  emit('close')
}
...
function onDrop(e: DragEvent) {
  e.preventDefault()
  isDragging.value = false
  if (isPhysicalBook.value) return
    ...
    try {
      const res = await api('/api/v1/books', {
        ...
        body: JSON.stringify({
          ...
          author: manualAuthor.value.trim() || undefined,
          isbn13: manualIsbn.value.trim() || undefined,
        }),
      })
      if (res.ok) {
        ...
        if (bookId) {
          manualUploadedIds.value.push(bookId)
        }
        manualSuccessCount.value = 1
        emitLibraryUploadCompleted({
          libraryId,
          uploadedBookIds: bookId ? [bookId] : [],
          uploadedCount: 1,
          failedCount: 0,
          attemptedCount: 1,
        })
    ...
    <div class="fixed inset-0 z-[70] flex items-center justify-center p-2 sm:p-4">
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          ...
          <!-- Folder selector -->
          <div v-if="folders.length > 1 && !isPhysicalBook" class="flex flex-col gap-1.5">
          ...
          <!-- Checkbox para Livro Físico / Manual -->
          <div class="flex items-center gap-2 py-1 px-1 bg-muted/20 rounded-md border border-border/50">
            <label class="flex items-center gap-2 text-xs font-medium text-foreground cursor-pointer w-full select-none px-1">
              <input type="checkbox" v-model="isPhysicalBook" class="rounded border-input text-primary focus:ring-ring" />
              <BookOpen :size="14" class="text-primary" />
              <span>{{ t('library.upload.physical.checkboxLabel') }}</span>
            </label>
          </div>

          <!-- Formulário para Inserção Manual de Livro Físico -->
          <div v-if="isPhysicalBook" class="flex flex-col gap-3 p-4 rounded-lg border border-border bg-muted/10">
            <p class="text-xs text-muted-foreground">{{ t('library.upload.physical.description') }}</p>
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-medium text-muted-foreground">{{ t('library.upload.physical.titleLabel') }}</label>
              <input
                v-model="manualTitle"
                type="text"
                :placeholder="t('library.upload.physical.titlePlaceholder')"
                class="h-8 rounded-md border border-input bg-background px-2.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
              />
            </div>
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-medium text-muted-foreground">{{ t('library.upload.physical.authorLabel') }}</label>
              <input
                v-model="manualAuthor"
                type="text"
                :placeholder="t('library.upload.physical.authorPlaceholder')"
                class="h-8 rounded-md border border-input bg-background px-2.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
              />
            </div>
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-medium text-muted-foreground">{{ t('library.upload.physical.isbnLabel') }}</label>
              <input
                v-model="manualIsbn"
                type="text"
                :placeholder="t('library.upload.physical.isbnPlaceholder')"
                class="h-8 rounded-md border border-input bg-background px-2.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
              />
            </div>
          </div>

          <!-- Fluxo padrão de upload de arquivos (exibido apenas se não for físico) -->
          <template v-else>
        ...
        <div v-if="allSuccess" class="shrink-0 px-5 py-4 border-t border-border flex items-center justify-between gap-3">
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm font-medium text-foreground">
              {{ isPhysicalBook ? t('library.upload.physical.successMsg') : t('library.upload.booksAdded', { count: doneCount }) }}
            </span>
          </div>
          <div class="flex items-center gap-2">
            <button
        ...
        <div v-else class="shrink-0 px-5 py-4 border-t border-border flex items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              ...
              :disabled="!canUpload || isSubmittingManual"
              @click="handleUpload"
            >
              <Loader2 v-if="isSubmittingManual" :size="13" class="animate-spin" />
              <Upload v-else :size="13" />
              {{
                isPhysicalBook
                  ? t('library.upload.physical.submitBtn')
                  : pendingCount > 0
                    ? t('library.upload.uploadWithCount', { count: pendingCount })
