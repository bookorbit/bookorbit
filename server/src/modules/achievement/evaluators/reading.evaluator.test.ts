...
describe('ReadingEvaluator', () => {
  ...
  describe('all_nighter', () => {
    it('awards all_nighter for 2am-5am session in UTC', async () => {
      const awards = await evaluator.evaluate(
        {
          userId: 1,
          eventName: ACHIEVEMENT_EVENT_READING_SESSION_SAVED,
          payload: makeSessionPayload({
            startedAt: new Date(Date.UTC(2026, 2, 15, 3, 0, 0)),
            endedAt: new Date(Date.UTC(2026, 2, 15, 3, 30, 0)),
          }) as never,
        },
        new Set(),
      );
    ...
    it('awards all_nighter using local timezone when UTC hour is outside range', async () => {
      // 6am UTC = 2am EDT (America/New_York in March = UTC-4)
      const awards = await evaluator.evaluate(
        {
          userId: 1,
          eventName: ACHIEVEMENT_EVENT_READING_SESSION_SAVED,
          payload: makeSessionPayload({
            ...
            endedAt: new Date('2026-03-15T06:30:00Z'),
    ...
    it('does not award all_nighter when local timezone puts session outside 2am-5am', async () => {
      // 6am UTC is 6am UTC - not in 2am-5am range
      const awards = await evaluator.evaluate(
        {
          userId: 1,
          eventName: ACHIEVEMENT_EVENT_READING_SESSION_SAVED,
  ...
  describe('early_bird', () => {
    it('awards early_bird for session starting between 4am and 7am in UTC', async () => {
      const awards = await evaluator.evaluate(
        {
          userId: 1,
          eventName: ACHIEVEMENT_EVENT_READING_SESSION_SAVED,
    ...
    it('does not award early_bird for session starting before 4am', async () => {
      const awards = await evaluator.evaluate(
        {
          userId: 1,
          eventName: ACHIEVEMENT_EVENT_READING_SESSION_SAVED,
