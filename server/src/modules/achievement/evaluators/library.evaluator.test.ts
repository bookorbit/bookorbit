...
describe('LibraryEvaluator', () => {
      ...
      const awards = await evaluator.evaluate(
        {
          ...
          payload: { userId: 1, libraryId: 10 } as never,
        },
        new Set(),
      );
      expect(awards).toContainEqual(expect.objectContaining({ key: 'library_builder_1' }));
  ...
  describe('annotator tiers', () => {
    it('awards annotator_1 at 10 annotations', async () => {
      ...
      const awards = await evaluator.evaluate(
        { userId: 1, isSuperuser: false, timeZone: 'UTC', eventName: ACHIEVEMENT_EVENT_ANNOTATION_CREATED, payload: { userId: 1, bookId: 1 } },
        new Set(),
      );
      ...
      const awards = await evaluator.evaluate(
        {
          ...
          timeZone: 'UTC',
          eventName: ACHIEVEMENT_EVENT_LIBRARY_CATALOG_CHANGED,
          payload: { userId: 1, libraryId: 2 } as never,
        },
        new Set(),
      );
      expect(awards).toContainEqual(expect.objectContaining({ key: 'multi_format' }));
  ...
  describe('curator', () => {
    it('awards curator at 10 collections', async () => {
      ...
      const awards = await evaluator.evaluate(
        { userId: 1, isSuperuser: false, timeZone: 'UTC', eventName: ACHIEVEMENT_EVENT_COLLECTION_CREATED, payload: { userId: 1 } },
        new Set(),
      );
      ...
      const awards = await evaluator.evaluate(
        {
          ...
          payload: { userId: 1, bookId: 1, annotationId: 9 },
        },
        new Set(),
      );
