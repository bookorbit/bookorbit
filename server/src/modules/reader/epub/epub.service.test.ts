...
// Some toolchains (e.g. Python's xml.etree.ElementTree) round-trip namespaced OPFs with
// auto-generated prefixes instead of a fixed opf:/dc:, and can bind the same namespace to
// more than one prefix in the same document. Prefixed attributes need to survive too.
const OPF_XML_AUTOGENERATED_PREFIX = `
<ns0:package xmlns:ns0="http://www.idpf.org/2007/opf" version="1.0">
  <ns0:metadata>
    <dc:title xmlns:dc="http://purl.org/dc/elements/1.1/">Reader Test</dc:title>
    <dc:creator xmlns:dc="http://purl.org/dc/elements/1.1/" ns0:role="aut" ns0:file-as="Doe, Jane">Jane Doe</dc:creator>
    <ns1:description xmlns:ns1="http://purl.org/dc/elements/1.1/">A sample EPUB</ns1:description>
    <ns0:meta name="cover" content="cover-image" />
  </ns0:metadata>
  <ns0:manifest>
    <ns0:item id="chap1" href="text/ch1.xhtml" media-type="application/xhtml+xml" />
    <ns0:item id="ncx" href="toc.ncx" media-type="application/x-dtbncx+xml" />
    <ns0:item id="cover-image" href="images/cover.jpg" media-type="image/jpeg" />
  </ns0:manifest>
  <ns0:spine>
    <ns0:itemref idref="chap1" />
  </ns0:spine>
</ns0:package>
`;
...
describe('EpubService', () => {
  ...
  it('parses an OPF whose elements use auto-generated namespace prefixes instead of opf:/dc:', async () => {
    mockOpenFile.mockResolvedValueOnce(
      makeArchive([
        { path: 'META-INF/container.xml', content: CONTAINER_XML },
        { path: 'OPS/content.opf', content: OPF_XML_AUTOGENERATED_PREFIX },
        { path: 'OPS/toc.ncx', content: NCX_XML },
        { path: 'OPS/text/ch1.xhtml', content: '<h1>ch1</h1>' },
      ]) as any,
    );
    ...
    expect(info.metadata).toEqual(expect.objectContaining({ title: 'Reader Test', creator: 'Jane Doe', description: 'A sample EPUB' }));
    expect(info.manifest.map((m) => m.id)).toEqual(['chap1', 'ncx', 'cover-image']);
    expect(info.spine.map((s) => s.href)).toEqual(['OPS/text/ch1.xhtml']);
    expect(info.coverPath).toBe('OPS/images/cover.jpg');
  });
