...
describe('KoreaderCatalogController', () => {
  ...
  it('forwards catalog routes to the service', async () => {
    ...
    await expect(controller.sections(user, 'libraries')).resolves.toEqual({ section: 'libraries', items: [] });
    ...
    expect(catalogService.getSectionEntries).toHaveBeenCalledWith(user, 'libraries');
