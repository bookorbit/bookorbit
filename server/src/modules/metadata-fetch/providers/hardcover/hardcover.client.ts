...
export class HardcoverClient {
  ...
  async searchByIsbn(isbn: string, apiKey: string): Promise<HardcoverBookWithEditions[]> {
    const body = await this.post<HardcoverBooksResponse>(SEARCH_BY_ISBN_QUERY, { isbn }, apiKey);
    return body?.data?.books ?? [];
  }

  async searchBooks(query: string, apiKey: string): Promise<HardcoverSearchDocument[]> {
    const body = await this.post<HardcoverSearchResponse>(SEARCH_BOOKS_QUERY, { q: query, limit: DEFAULT_SEARCH_LIMIT }, apiKey);
  ...
  async lookupBySlug(slug: string, apiKey: string): Promise<HardcoverBookWithEditions | null> {
    const body = await this.post<HardcoverBooksResponse>(LOOKUP_BY_SLUG_QUERY, { slug }, apiKey);
    return body?.data?.books?.[0] ?? null;
  }

  private async post<T>(query: string, variables: Record<string, unknown>, apiKey: string): Promise<T | null> {
    ...
    try {
      ...
      if (!res.ok) {
        this.logger.warn(`Hardcover API returned ${res.status}`);
        return null;
      }
      ...
      this.logger.warn(`Hardcover API request failed: ${(err as Error).message}`);
      return null;
    }
  }
}
